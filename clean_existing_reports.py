    ...
    try:
        ...
        with engine.connect() as connection:
            ...
            try:
                ...
                logger.info("Checking for 'dangerous_content' in report_metadata...")
                metadata_query = text("""
                    SELECT id, report_metadata FROM report 
                    WHERE report_metadata::text LIKE '%dangerous_content%'
                """)
                ...
                for report in metadata_reports:
                    ...
                    try:
                        ...
                        logger.error(f"Invalid JSON in report_metadata for {report_id}, skipping")
                        continue
                    ...
                    if fixed_metadata:
                        # Update the report_metadata in the database
                        update_metadata_query = text("""
                            UPDATE report
                            SET report_metadata = :metadata
                            WHERE id = :id
                        """)
                        connection.execute(update_metadata_query, {"id": report_id, "metadata": json.dumps(metadata)})
                        logger.info(f"Updated report_metadata for report {report_id}")
